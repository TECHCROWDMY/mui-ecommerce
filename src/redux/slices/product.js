...
export function getProducts() {
  return async (dispatch) => {
    ...
    try {
      const response = await axios.get('https://api-prod-minimal-v700.vercel.app/api/product/list');
...
export function getProduct(id) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios.get('https://api-prod-minimal-v700.vercel.app/api/product/details', {
        params: { productId: id },
      });
